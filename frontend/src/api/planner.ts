...
export interface Intervention { id: number; concept: string; status: 'suggested' | 'monitoring' | 'needs_instructor' | 'resolved' | 'dismissed'; reason: string; baseline_score: number | null; latest_score: number | null; followup_score: number | null; instructor_note: string | null; change: number | null; reviewed_at: string | null; evidence?: { basis?: string; risk_flag?: { risk_score: number; severity: string; reasons: string[]; computed_at: string | null } | null; history?: unknown[] } }
...
export interface RiskInterventionInput { course_id: number; user_id: number; note?: string; concept?: string | null }
export interface RiskIntervention extends QueueIntervention { created: boolean; goal_created: boolean }
...
export const plannerAPI = {
  ...
  createFromRisk: async (input: RiskInterventionInput) => (await api.post<RiskIntervention>('/planner/instructor/risk-interventions', input)).data,
