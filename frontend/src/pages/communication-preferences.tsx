...
export function NotificationTopicPreferencesCard() {
  const query = useQuery({
    queryKey: ["notification-topic-preferences"],
  ...
  async function update(
    ...
    setSavingKey(key);
    try {
      await notificationsApi.updatePreference(topic, { [field]: value });
      await query.refetch();
  ...
  if (query.isLoading) {
    return (
      <section className="campus-panel" aria-label="Loading notification preferences">
        <div className="campus-skeleton" role="status" />
      </section>
    );
  }
  ...
  return (
    <section className="campus-panel">
      <div className="campus-panel-heading">
        <div>
          <span className="campus-eyebrow">
            <Bell size={14} /> Notification topics
          </span>
          <h2 className="mt-2 text-xl font-bold text-secondary-900">
            Learning and account alerts
          </h2>
          <p className="campus-muted mt-2 max-w-2xl">
            These settings control the platform alerts that are generated by
            learning plans, course activity and SashaInfinity operations.
          </p>
        </div>
      </div>

      <div className="mt-5 divide-y divide-orange-100 overflow-hidden rounded-2xl border border-orange-100 bg-white/75">
        {query.data.topics.map((topic) => (
          ...
          >
            <div>
              ...
              <p className="campus-muted mt-1">{topic.description}</p>
            </div>
                ...
                return (
                  ...
                  >
                    <input
                      ...
                      onChange={(event) =>
                        void update(topic.topic, field, event.target.checked)
                      }
                    />
                    {label}
                  </label>
...
export function CommunicationPreferencesPage() {
  ...
  return (
    <div className="campus mx-auto max-w-6xl py-2">
      ...
      <NotificationTopicPreferencesCard />
